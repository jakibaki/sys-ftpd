...
#endif
#if !defined strncmp && !defined PORTABLE_strncmp
...
#if defined PORTABLE_strncmp
int strncmp(const TCHAR *s1, const TCHAR *s2, size_t n)
{
  while (n-- != 0 && (*s1 || *s2)) {
    register int c1, c2;
    c1 = *s1++;
  ...
  if (len > 0 || idxSection >= 0) {
    ...
    do {
      ...
      ep = _tcsrchr(sp, ']');
             ...
             (((int)(ep-sp-1) != len || Section == NULL || _tcsncmp(sp+1,Section,len) != 0) && ++idx != idxSection));
    if (idxSection >= 0) {
      if (idx == idxSection) {
        assert(ep != NULL);
        assert(*ep == ']');
  ...
  do {
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
           ...
           || ((len == 0 || (int)(skiptrailing(ep,sp)-sp) != len || _tcsncmp(sp,Key,len) != 0) && ++idx != idxKey));
  if (idxKey >= 0) {
    if (idx == idxKey) {
      assert(ep != NULL);
      assert(*ep == '=' || *ep == ':');
...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename)
{
  ...
  if (len > 0) {
    do {
      ...
      sp = skipleading(LocalBuffer);
      ep = _tcsrchr(sp, ']');
      match = (*sp == '[' && ep != NULL && (int)(ep-sp-1) == len && _tcsncmp(sp + 1,Section,len) == 0);
      if (!match || Key != NULL) {
  ...
  for( ;; ) {
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
    match = (ep != NULL && len > 0 && (int)(skiptrailing(ep,sp)-sp) == len && _tcsncmp(sp,Key,len) == 0);
