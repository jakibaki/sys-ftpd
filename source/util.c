...
void pauseInit() {
    ...
    FILE *should_pause_file = fopen("/config/ftpd/ftpd_paused", "r");
...
void setPaused(bool newPaused) {
    ...
    paused = newPaused;
    if(paused) {
        FILE *should_pause_file = fopen("/config/ftpd/ftpd_paused", "w");
        fclose(should_pause_file);
        playMp3("/config/ftpd/pauseon.mp3");
    } else {
        unlink("/config/ftpd/ftpd_paused");
        playMp3("/config/ftpd/pauseoff.mp3");
    }
    mutexUnlock(&pausedMutex);
}
